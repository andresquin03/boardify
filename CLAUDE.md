...
- Catalogo de juegos con infinite scroll: `src/lib/games-query.ts` centraliza tipos y helpers de filtros/orden (sin `"use server"`); `src/lib/games-actions.ts` exporta solo `fetchGamesPage` (server action); `src/components/games/games-grid.tsx` es el client component con `IntersectionObserver` + sentinel div. Reset de estado al cambiar filtros via `key={filterKey}` en `GamesPage`. IMPORTANTE: en archivos `"use server"` solo se pueden exportar funciones async; constantes, tipos y helpers sincrónicos deben ir en modulos separados.
- Observabilidad: `@vercel/analytics` y `@vercel/speed-insights` montados en `src/app/layout.tsx` (plan gratuito de Vercel).
